...
void sim_entries(GtkWidget *widget, gpointer data);
...
gui_thread (gpointer data)
{
  ...
  *new,*sep1,*sep2, *open,*save,*dialog, *save_as,*export,*export_diag, *quit, *sep, *tog_stat,*statusbar, *tog_tool;
  ...
  g_signal_connect(G_OBJECT(export), "activate",G_CALLBACK(file_export),(gpointer) window);
  /* Create vertical seperator line in the file menu */
  ...
  /*unset focus of the button*/
  GTK_WIDGET_UNSET_FLAGS(item_1, GTK_CAN_FOCUS);
  GTK_WIDGET_UNSET_FLAGS(item_2, GTK_CAN_FOCUS);
  GTK_WIDGET_UNSET_FLAGS(item_3, GTK_CAN_FOCUS);
  GTK_WIDGET_UNSET_FLAGS(item_4, GTK_CAN_FOCUS);
  GTK_WIDGET_UNSET_FLAGS(item_5, GTK_CAN_FOCUS);


  /*Signal Connects for the Toolbar*/
  g_signal_connect(G_OBJECT(item_1), "clicked",G_CALLBACK(sim_entries), item_1);
...
static void file_export(GtkWidget *widget,  GtkWindow *window)
{
	GtkWidget *dialog, *table, *export_file_name;
    GtkWidget *lbl1;
    gchar *file_name="export.svg";
    gint result;

	dialog = gtk_dialog_new_with_buttons ("Export to svg-File", NULL,GTK_DIALOG_MODAL,GTK_STOCK_OK, GTK_RESPONSE_OK,
    GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL,NULL);
    
    gtk_dialog_set_default_response (GTK_DIALOG (dialog), GTK_RESPONSE_OK);
    /* Create the entrie for the new file nam,e*/
    lbl1 = gtk_label_new ("Enter Name for the svg File:");
    export_file_name = gtk_entry_new();
    //gtk_entry_set_text(export_file_name,"default.svg");
...
//gtk_entry_set_text (GTK_ENTRY (new_file_name), g_get_new_file_name_name());
  ...
  table = gtk_table_new (1, 2, FALSE);
  gtk_table_attach_defaults (GTK_TABLE(table),lbl1,0,1,0,1);
  gtk_table_attach_defaults (GTK_TABLE(table),export_file_name,1,2,0,1);
  gtk_table_set_row_spacings (GTK_TABLE (table), 5);
  gtk_table_set_col_spacings (GTK_TABLE (table), 5);
  gtk_container_set_border_width (GTK_CONTAINER (table), 5);
  gtk_box_pack_start_defaults (GTK_BOX (GTK_DIALOG (dialog)->vbox), table);
  surface = cairo_svg_surface_create(file_name, width+50, height+50);
...
g_print ("File Name: %s\n", gtk_entry_get_text (GTK_ENTRY (export_file_name)));
file_name=(void*)gtk_entry_get_text (GTK_ENTRY (export_file_name));
}
gtk_widget_destroy (dialog);
...
{
  if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widget))) {
      gtk_window_set_title(window, "R1 Button active");	
  } else {
      gtk_window_set_title(window, "");
  }
}
...
{
  if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widget))) {
      gtk_window_set_title(window, "R2 Button active");	
  } else {
      gtk_window_set_title(window, "");
  }
}
...
{
  if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widget))) {
      gtk_window_set_title(window, "R3 Button active");	
  } else {
      gtk_window_set_title(window, "");
  }
}
...
{
  if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widget))) {
      gtk_window_set_title(window, "R4 Button active");	
  } else {
      gtk_window_set_title(window, "");
  }
}
...
/*################################################################*/
 /* Entries Dialog for the Simulator Mode*/
/*###############################################################*/
void sim_entries(GtkWidget *widget, gpointer data)
{
	GtkWidget *dialog, *table, *Port1_Val, *Port2_Val, *Port3_Val,*R1_Val,*R2_Val,*R3_Val,*R4_Val;
    GtkWidget *lbl1,*lbl2,*lbl3,*lbl4,*lbl5,*lbl6,*lbl7,*lbl8;
    gint result;

	dialog = gtk_dialog_new_with_buttons ("Simulation Values", NULL,GTK_DIALOG_MODAL,GTK_STOCK_EXECUTE, GTK_RESPONSE_OK,
    GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL,NULL);
    
    gtk_dialog_set_default_response (GTK_DIALOG (dialog), GTK_RESPONSE_OK);
    /* Create the entrie for the new file nam,e*/
    lbl1 = gtk_label_new ("Enter Values to simulate:");
    lbl2 = gtk_label_new ("Port B:");
    lbl3 = gtk_label_new ("Port C:");
    lbl4 = gtk_label_new ("Port D:");
    lbl5 = gtk_label_new ("R1:");
    lbl6 = gtk_label_new ("R2:");
    lbl7 = gtk_label_new ("R3:");
    lbl8 = gtk_label_new ("R4:");
    Port1_Val = gtk_entry_new();
    Port2_Val = gtk_entry_new();
    Port3_Val = gtk_entry_new();
    R1_Val = gtk_entry_new();
    R2_Val = gtk_entry_new();
    R3_Val = gtk_entry_new();
    R4_Val = gtk_entry_new();
/*Option for default values, currently not use*/
//gtk_entry_set_text (GTK_ENTRY (new_file_name), g_get_new_file_name_name());

table = gtk_table_new (8, 2, FALSE);
/*Attach labels in the dialog box*/
gtk_table_attach_defaults (GTK_TABLE(table),lbl1,0,1,0,1);
...
gtk_table_attach_defaults (GTK_TABLE(table),lbl7,0,1,6,7);
gtk_table_attach_defaults (GTK_TABLE(table),lbl8,0,1,7,8);
/*Attach entrie boxes in the dialog*/
gtk_table_attach_defaults (GTK_TABLE(table),Port1_Val,1,2,1,2);
gtk_table_attach_defaults (GTK_TABLE(table),Port2_Val,1,2,2,3);
gtk_table_attach_defaults (GTK_TABLE(table),Port3_Val,1,2,3,4);
gtk_table_attach_defaults (GTK_TABLE(table),R1_Val,1,2,4,5);
gtk_table_attach_defaults (GTK_TABLE(table),R2_Val,1,2,5,6);
gtk_table_attach_defaults (GTK_TABLE(table),R3_Val,1,2,6,7);
gtk_table_attach_defaults (GTK_TABLE(table),R4_Val,1,2,7,8);
...
if (result == GTK_RESPONSE_OK)
{
g_print ("Port B: %s\n", gtk_entry_get_text (GTK_ENTRY (Port1_Val)));
g_print ("Port C: %s\n", gtk_entry_get_text (GTK_ENTRY (Port2_Val)));
g_print ("Port D: %s\n", gtk_entry_get_text (GTK_ENTRY (Port3_Val)));
}
   gtk_widget_destroy (dialog);
}
