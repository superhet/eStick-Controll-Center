...
/*!
 * \brief some weird data generator ...
 */
static gboolean data_handler (gpointer data)
{
  static gint j;
...
gui_thread (gpointer data)
{
  ...
  *new,*sep1,*sep2, *open,*save,*dialog, *save_as,*export, *quit, *sep, *tog_stat,*statusbar, *tog_tool;
  ...
  g_signal_connect(G_OBJECT(pref_1), "activate",G_CALLBACK(preferences),preferences);  
...
void show_help(GtkWidget *widget, gpointer window)
{
  ...
  gtk_about_dialog_set_name(GTK_ABOUT_DIALOG(dialog), "Online Help");
...
//static void file_save_as (GtkButton *button,GtkWindow *window)
static void file_save_as (GtkWidget *widget, GtkWindow *window)
{
   GtkWidget *dialog;
   gchar *filename;
   ...
   dialog = gtk_file_chooser_dialog_new ("Save File As ...", window,
                                         ...
                                         NULL);
    /*Set File Filter for File Save As Dialog*/
   filter = gtk_file_filter_new ();
   gtk_file_filter_set_name(filter,"log");
   gtk_file_filter_add_pattern (filter, "*.log");
   gtk_file_chooser_add_filter (GTK_FILE_CHOOSER (dialog),filter);    
   /*The Build and add Sequence of the Filter Ends here*/
